...
package ly.neptune.nexus.fcms.fxhousesclient.exchangerates.internal
...
import ly.neptune.nexus.fcms.fxhousesclient.exchangerates.FcmsFxExchangeRatesClient
...
internal class FcmsFxExchangeRatesClientImpl(
    private val config: FcmsFxConfig,
) : FcmsFxExchangeRatesClient {
    ...
    override suspend fun list(page: Int?, options: RequestOptions?): Page<ExchangeRate> {
        val base = effectiveBaseUrl(options)
        val url = buildString {
            append(base)
            append("/api/v1/exchange-rates")
