...
package ly.neptune.nexus.fcms.fxhousesclient.requests.internal
...
import ly.neptune.nexus.fcms.fxhousesclient.requests.FcmsFxRequestsClient
...
internal class FcmsFxRequestsClientImpl(
    private val config: FcmsFxConfig,
) : FcmsFxRequestsClient {
    ...
    override suspend fun listPendingRequests(page: Int?, options: RequestOptions?): Page<PurchaseRequestQueueItem> {
    ...
    override suspend fun refreshQueue(options: RequestOptions?): RefreshQueueResult {
        ...
        val body = executeWithRetries(req, isIdempotent = false)
        body.use { rb ->
            return JsonSupport.readSingleEnvelope(rb.byteStream(), RefreshQueueResult::class.java)
        }
    }
